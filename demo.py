...
if args.flag_test == 'test':
    ...
    tic = time.time()
    best_acc = 0.0
    # PATH = "./log2/best_model_SpectralFormer_patch_pretrain.pt"
    # pretrained_dict = torch.load(PATH)
    # model_dict = model.state_dict()
    #只加载和当前网络参数命名相同的预训练的参数
    ...
    for epoch in range(args.epoches):
        ...
        # #print("Epoch: {:03d} test_OA: {:.4f} AA: {:.4f} kappa: {:.4f} AA_whole: {:.4f} AF {:}".format(epoch + 1,
                                                                                                       # OA2,
                                                                                                          # AA_mean2,
                                                                                                          # Kappa2,
                                                                                                          # AA2[-1],
                                                                                                          # 'D'if args.device == '0' else ''))
        scheduler.step()                                                             
    toc = time.time()
